...
特色：可以实现b站富文本的渲染，图片自适应，挂件，大会员，认证号，投稿，专栏的渲染
    ...
    平均 0.7 秒 ，有缓存个别 0.2s
        ...
        BUG 发现一例带有互动抽奖的动态后at的分割数据出现一位偏差，但不影响阅读
        已解决 文本分割器逻辑性错误，遇到连续关键字出现漏分割
...
WeiRuanYaHei = bsepth + r'WeiRuanYaHei-1.ttf'
NotoColorEmoji = bsepth + r'NotoColorEmoji.ttf'
CODE2000 = bsepth + r'CODE2000.ttf'


rtime = []
mfont = TTFont(WeiRuanYaHei)
...
class DynamicPictureRendering:
    ...
    def get_font_render_size(self, FOUNT, fontsize, text):
        ...
        try:
            ...
            logger.debug(f'字符{text}宽度计算失败，已使用默认值')
    ...
    def AoutLine(self, limt, text, size, color='#000000', fount=WeiRuanYaHei, x=15, y=0):
        """非富文本样式计算"""
        START_X = x
        START_Y = y
        SZ = 0
    ...
    async def headRendering(self, desc):
        ...
        facePath = bsepth + f'face/{fm}'
        pendantPath = bsepth + f'pendant/{pendant.pid}.png'
        ...
        if pendant.image != "":
            ...
            if pendantimg:
                ...
                pendant = Image.open(
                    bsepth + f'pendant/{pendant.pid}.png').resize((ppx, ppx), Image.ANTIALIAS).convert('RGBA')
    ...
    async def NGSSTrcker(
        ...
        while count != len(NGSS):
            ...
            if NGSS[count]['type'] == 2 and NGSS[count]['data']['control'] == 2:
                data = {
                    'type': 3, 'text': Text[:NGSS[count]['start']], "data": 'element/box.png'}
                RenderList.append(data)
                ...
                data = {
                    'type': 3, 'text': Text[:NGSS[count]['start']], "data": 'element/tick.png'}
                RenderList.append(data)
                data = {'type': NGSS[count]['type'], 'text': Text[NGSS[count]
                                                                  ['start']+1:NGSS[count]['end']], "data": NGSS[count]['data']}
            elif NGSS[count]['type'] == 2 and NGSS[count]['data']['control'] == 5:
                data = {'type': 3, 'text': '', "data": 'element/link.png'}
                RenderList.append(data)
        ...
        if len(pl) != 0:
            ...
            tasks = []
            for id in keys:
                if not os.path.exists(bsepth + f'emoji/{id}.png'):
                    tasks.append(asyncio.create_task(self.getPage(
                        emojiid[id], bsepth + f'emoji/{id}.png', 2, id)))
            ...
            for emoji in pl:
                ...
                if id in emlist:
                    ...
                    emimg = Image.open(
                        bsepth + f'emoji/{id}.png').resize((FOUNT_SIZE+10, FOUNT_SIZE+10), Image.ANTIALIAS).convert('RGBA')
                    emlist[id] = emimg
    ...
    async def FunctionBlock(self, type, card, background="#FFFFFF"):
        ...
        if type in [4099, 4308, 8]:
            ...
            for t in title:
                wihdt = self.get_font_render_size(WeiRuanYaHei, 25, t)[0]
                textcount += 1
                allwidth += wihdt
                if allwidth > 675:
        ...
        # 相册
            ...
            titlestly = self.AoutLine(
                675, title, 30, color='#000000', fount=WeiRuanYaHei)
            ...
            if summary != '':
                summstly = self.AoutLine(
                    675, summary, 25, color='#666666', fount=WeiRuanYaHei, y=lastpic+20)
    ...
    async def AddCard(self, display):
        ...
        if display.add_on_card_info:
            ...
            if type == 6:
                ...
                img = Image.new("RGB", (700, 150), "#f4f5f7")
                img_draw = ImageDraw.Draw(img)
                nicknameFont = ImageFont.truetype(WeiRuanYaHei, 25)
                img_draw.text((40, 35), title,
                              font=nicknameFont, fill="#000000")
                nicknameFont = ImageFont.truetype(WeiRuanYaHei, 20)
    ...
    async def Reprender(self, desc, card, display):
        ...
        if type == 1:
            ...
            # 头部信息字体
            MainFont = ImageFont.truetype(WeiRuanYaHei, 28)
...
data = {
    "desc": {
        ...
        "like": 3946,
        "is_liked": 0,
        "dynamic_id": 533536432542407600,
        "timestamp": 1623062031,
        "pre_dy_id": 0,
        "orig_dy_id": 0,
        "orig_type": 0,
        "user_profile": {
            ...
            "rank": "10000",
                    "sign": "爱折腾、超硬核的科技数码频道！商务合作请联系：bd@geekerwan.com",
                    "level_info": {
                        "current_level": 6
            }
        },
        ...
        "inner_id": 0,
        "status": 1,
        "dynamic_id_str": "533536432542407644",
        "pre_dy_id_str": "0",
        "orig_dy_id_str": "0",
        "rid_str": "138133716"
    },
    "card": "{\"item\":{\"at_control\":\"\",\"category\":\"daily\",\"description\":\"今晚1点，直播陪你们看苹果WWDC大会。还是老样子，播得了就播，播不了就听我们bb吧[doge]\",\"id\":138133716,\"is_fav\":0,\"pictures\":[{\"img_height\":631,\"img_size\":631.96,\"img_src\":\"https:\\/\\/i0.hdslb.com\\/bfs\\/album\\/9e561ba1fb48a72663f15cee351d8adac2d78eb2.png\",\"img_tags\":null,\"img_width\":927}],\"pictures_count\":1,\"reply\":433,\"role\":[],\"settings\":{\"copy_forbidden\":\"0\"},\"source\":[],\"title\":\"\",\"upload_time\":1623062031},\"user\":{\"head_url\":\"https:\\/\\/i1.hdslb.com\\/bfs\\/face\\/d0f7a7ee34a4a45c8390eb3a07e4d7f2d70bae91.jpg\",\"name\":\"极客湾Geekerwan\",\"uid\":25876945,\"vip\":{\"avatar_subscript\":1,\"due_date\":1650729600000,\"label\":{\"label_theme\":\"annual_vip\",\"path\":\"\",\"text\":\"年度大会员\"},\"nickname_color\":\"#FB7299\",\"status\":1,\"theme_type\":0,\"type\":2,\"vip_pay_type\":0}}}",
            ...
            "display": {
                ...
                "comment_info": {
                    ...
                    "comment_ids": "4687206674"
                }
    }
}
if __name__ == "__main__":
    data2 = DynamicCard(**data)
    Render = DynamicPictureRendering(data2)
